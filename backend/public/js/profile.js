class Profile {
    ...
    async getCurrentUser(username) {
        const data = await fetch(`https://teclanetwork.azurewebsites.net/app/profile-data/${username}`);
    ...
    async renderFriends() {
        ...
        for (let i = 0; i < json.length; i++) {
            ...
            //obtenemos nombre e imagen
            const dataOrigen = await fetch(`https://teclanetwork.azurewebsites.net/profile-data/${email_ori}`);
